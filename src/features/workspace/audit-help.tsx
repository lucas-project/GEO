...
export function AuditHelpBlurb({ className = '' }: { className?: string }) {
  return (
    <p className={`text-[13px] text-fg-subtle leading-relaxed ${className}`}>
...
export function LatestAuditReportLink({
  ...
  return (
    <p className={`text-[13px] text-fg-subtle ${className}`}>
      Latest report for this workspace:{' '}
      <Link href={`/audit/${auditId}`} className="text-accent hover:underline font-mono text-[12px]">
        View audit report
      </Link>
    </p>
  );
...
export function AuditReportIdFieldHelp() {
  return (
    <p className="text-[13px] text-fg-subtle leading-relaxed mt-1.5">
      ...
      <span className="font-mono text-[12px]">/audit/</span> (example:{' '}
      <span className="font-mono text-[12px]">clx1abc2def3</span>).
    </p>
  );
}
