...
export async function POST(req: Request) {
  ...
  try { return NextResponse.json({ evidence: await addUserEvidence(input.data, ownerId) }, { status: 201 }); }
...
export async function GET(req: Request) {
  ...
  try { return NextResponse.json({ evidence: await readUserEvidence(siteId, ownerId) }); }
